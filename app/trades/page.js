  ...
  export default function TradesPage() {
    ...
    useEffect(() => {
      fetch('/api/setup-eye/log')
        .then(r => r.json())
        .then(d => { if (d.entries?.length) setSetupEyeLog(d.entries); })
        .catch(() => {});
        ...
        const fetchData = async () => {
          try {
            ...
            try {
              ...
              const heResult   = runSetupEye(data.candles, vwapForHE, rsiVal, setupEyeEnvRef.current);
    ...
    // Setup Eye: plain-English narrative builder for each candle entry
          ...
          {/* OLD: <div className="mb-4 bg-gradient-to-r from-purple-900/50 via-blue-900/50 to-purple-900/50 border border-purple-700/50 rounded-xl backdrop-blur-sm overflow-hidden"> */}
            {commentaryLoading && !commentary ? (
              ...
              {powerCandles.length > 0 && (() => {
                ...
                // Only show for today's candles (IST) — don't carry Friday's alert into Monday
