...
function PhaseCard({ symbol, data, loading }) {
  ...
  if (loading && !data) return (
    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 flex items-center gap-3 animate-pulse">
  ...
  if (!data || data.error) return (
    <div className="rounded-xl border border-white/8 bg-white/[0.02] p-4">
      <div className="text-xs text-slate-600">{symbol} — unavailable</div>
    </div>
  );
  ...
  return (
    <div className={`rounded-xl border ${meta.border} ${meta.bg} p-4`}>
      ...
      {/* Phase description */}
      <p className="text-[11px] text-slate-400 mb-3 leading-relaxed">{meta.description}</p>
      ...
      {(data.ema20 || data.ema50) && (
        <div className="flex items-center gap-4 text-[10px] text-slate-500 pt-2 border-t border-white/5 flex-wrap">
          {data.currentPrice && <span className="text-white font-mono">₹{data.currentPrice?.toLocaleString('en-IN')}</span>}
          {data.ema20 && (
            <span>
              EMA20 <span className={data.priceVsEma20 === 'ABOVE' ? 'text-green-400' : 'text-red-400'}>
          ...
          {data.ema50 && (
            <span>
              EMA50 <span className={data.priceVsEma50 === 'ABOVE' ? 'text-green-400' : 'text-red-400'}>
          ...
          {data.volumeBias && (
            <span className="ml-auto">
              Vol <span className={data.volumeBias === 'BULLISH' ? 'text-green-400' : data.volumeBias === 'BEARISH' ? 'text-red-400' : 'text-slate-400'}>
...
export default function MarketPhaseSection() {
  ...
  const envSummary   =
    bothMarkup   ? { text: 'Bullish environment — swing longs favoured',   color: 'text-green-400'  } :
    bothMarkdown ? { text: 'Bearish environment — swing shorts favoured',  color: 'text-red-400'    } :
    bothDist     ? { text: 'Distribution — reduce exposure, avoid new longs', color: 'text-orange-400' } :
    niftyPhase && bnfPhase ? { text: 'Mixed signals — size down, be selective', color: 'text-amber-400' } :
    null;
  ...
  return (
    <div className="mb-10">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-bold tracking-[0.15em] uppercase text-violet-400">Market Phase</span>
          </div>
      ...
      {envSummary && (
        <div className="mb-3 px-3 py-2 rounded-lg bg-white/[0.03] border border-white/8 text-xs font-medium">
