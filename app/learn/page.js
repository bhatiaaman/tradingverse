...
const ARTICLES = [
  { tag: 'Market Psychology', title: 'The trader who knew everything but lost anyway', readTime: '6 min', slug: 'trader-who-knew-everything', color: 'text-cyan-400' },
  { tag: 'Price Action', title: 'Why most breakout trades fail', readTime: '8 min', slug: 'why-breakouts-fail', color: 'text-blue-400' },
  { tag: 'Risk Management', title: 'The revenge trade trap — and how to escape it', readTime: '5 min', slug: 'revenge-trade-trap', color: 'text-rose-400' },
  { tag: 'Trade Review', title: 'Post-trade review: the habit that separates pros', readTime: '7 min', slug: 'post-trade-review', color: 'text-emerald-400' },
]
  ...
  return (
    <div className="min-h-screen bg-[#060b14] text-white">
      ...
      <div className="max-w-5xl mx-auto px-6 py-20">
        ...
        {/* Header */}
        <p className="text-violet-400 text-xs font-bold tracking-[0.2em] uppercase mb-4">Learn from Masters</p>
        ...
        {/* Tabs */}
        <div className="flex gap-1 p-1 bg-white/[0.03] rounded-xl border border-white/5 w-fit mb-12">
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-6 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200
                ${tab === t ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-slate-300'}`}>
              {t}
              {t === 'Views' && <span className="ml-2 text-[9px] font-bold tracking-wider uppercase bg-blue-900/50 text-blue-400 px-1.5 py-0.5 rounded-full border border-blue-800/60">Soon</span>}
            </button>
          ))}
        </div>
            ...
            <div className="grid md:grid-cols-2 gap-5">
              {BOOKS.map(b => (
                <Link key={b.slug} href={`/learn/books/${b.slug}`}
                  className="group flex flex-col p-7 rounded-2xl border border-white/10 bg-white/[0.02] hover:border-white/20 hover:bg-white/[0.04] hover:-translate-y-0.5 transition-all duration-200">

                  <div className="flex items-start justify-between mb-5">
                    <div className="flex items-center gap-4">
                      <span className="text-4xl">{b.emoji}</span>
                      <div>
                        <h3 className="font-bold text-white text-base leading-tight group-hover:text-blue-100 transition-colors">{b.title}</h3>
                  ...
                  <p className="text-slate-400 text-sm leading-relaxed mb-5 flex-1">{b.summary}</p>

                  <div className="flex flex-wrap gap-2 mb-5">
                    {b.lessons.map(l => (
                      <span key={l} className="text-[10px] text-slate-500 border border-white/8 px-2.5 py-1 rounded-full">
                        {l}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-white/5">
                    ...
                    <span className="text-xs text-slate-500 group-hover:text-blue-400 transition-colors font-semibold">Read summary →</span>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-5">
            {ARTICLES.map(a => (
              <Link key={a.slug} href={`/learn/articles/${a.slug}`}
                ...
                <h3 className="text-lg font-bold text-white mb-3 leading-tight group-hover:text-blue-100 transition-colors">{a.title}</h3>
                <div className="mt-auto flex items-center justify-between pt-4 border-t border-white/5">
                  <span className="text-xs text-slate-600">{a.readTime} read</span>
                  <span className="text-xs text-slate-500 group-hover:text-blue-400 transition-colors font-semibold">Read article →</span>
                </div>
              </Link>
            ))}
          </div>
        ...
        {tab === 'Views' && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h3 className="text-2xl font-bold mb-3">Market Views — Coming Soon</h3>
